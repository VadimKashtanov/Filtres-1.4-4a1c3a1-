...
static float filtre(uint inst0, uint depart, float * x, float * f, uint intervalle, uint type_norme, float _min, float _max) {
	...
	if (inst0 == 0) {
		...
		return fp_normalisation(expf(-s*s-d*d));
	} else if (inst0 == 1) {
		//
		float s = 0;
		float f_nouveau = f[0];
		...
		return fpt_normalisation(expf(-s*s));
	} else {
		ERR("Pas possible")
	}
	return 0;
		...
		if (insts[c] == DOT1D_TANH) {
			...
			FOR(0, i, Y[c]) {
				FOR(DEPART, t, PRIXS_bitget) {
					...
					y_nouveau[t*MAX_Y + i] = tanh(s);//tanh(s);
				};
			};
			...
			ATTENDRE_CUDA();*/
			//
		} else if (insts[c] == DOT1D_LOGISTIC) {
			uint X = Y[c-1];
			//#pragma omp parallel
			//#pragma omp for
			FOR(0, i, Y[c]) {
				FOR(DEPART, t, PRIXS_bitget) {
					float s = poids[c][(X+1)*i + X-1+1];
					FOR(0, j, X) s += poids[c][(X+1)*i + j] * y_avant[t*MAX_Y + j];
					y_nouveau[t*MAX_Y + i] = 1.0 / (1 + expf(-s));
				};
			};
